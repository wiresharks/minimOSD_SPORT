...
extern const uint8_t MAXSCREENITEMS = 26; // 28 w/ GPS alt & Temperature
...
PROGMEM const char * const item_table[] = { screen_item_00, screen_item_01, screen_item_02, screen_item_03,
  screen_item_04, screen_item_05, screen_item_06, screen_item_07, screen_item_08, screen_item_09, screen_item_10,
  screen_item_11, screen_item_12, screen_item_13, screen_item_14, screen_item_15, screen_item_16, screen_item_17,
  screen_item_18, screen_item_19, screen_item_20, screen_item_21, screen_item_22, screen_item_23, screen_item_24,
  screen_item_25, screen_item_26,
//screen_item_27,
  };
...
void setMspRequests()
{
  ...
  if (Settings[S_MAINVOLTAGE_VBAT] || Settings[S_VIDVOLTAGE_VBAT] || Settings[S_MWRSSI])
...
void loop()
{
  ...
  if (Settings[S_ENABLEADC]) {
    ...
    if (!Settings[S_MWAMPERAGE]) {
      ...
      if (amperageADC > currsensOffSet)
        amperageADC = ((amperageADC - currsensOffSet) * 4.8828) / Settings[S_CURRSENSSENSITIVITY]; // [A] Positive Current flow (512...1023) or Unidir (0...1023)
      else
        amperageADC = ((currsensOffSet - amperageADC) * 4.8828) / Settings[S_CURRSENSSENSITIVITY]; // [A] Negative Current flow (0...512)
    }
#endif
  }
  ...
  if ((currentMillis - previous_millis_high) >= hi_speed_cycle) // 20 Hz (Executed every 50ms)
...
#ifdef FEATURE_AMPERAGE
    if (!Settings[S_MWAMPERAGE]) {
      calculateAmperage(); // Amperage and amperagesum integration on 50msec
    }
#endif
    ...
    if (allSec < 6) {
      ...
      if (fontMode) {
        ...
        displayMode();
#ifdef FEATURE_TEMPERATURE
        if((temperature<Settings[S_TEMPERATUREMAX])||(BlinkAlarm)) {
          displayTemperature();
        }
#endif
#ifdef FEATURE_AMPERAGE
