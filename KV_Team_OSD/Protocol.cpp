...
void handleRawRC()
{
  ...
  if (MwRcData[PITCHSTICK] > 1300 && MwRcData[PITCHSTICK] < 1700 && MwRcData[ROLLSTICK] > 1300
    && MwRcData[ROLLSTICK] < 1700 && MwRcData[YAWSTICK] > 1300 && MwRcData[YAWSTICK] < 1700) {
    waitStick = 0;
    timeout = 1000;
  }
  else if (waitStick == 1) {
    ...
    if ((MwRcData[PITCHSTICK] > MAXSTICK) && (MwRcData[YAWSTICK] > MAXSTICK) && (MwRcData[THROTTLESTICK] > MINSTICK)
      ...
      if (!previousarmedstatus && configMode && (MwRcData[THROTTLESTICK] < MINSTICK)) // EXIT
        ...
        if (configPage == 3 && COL == 3) {
          if (ROW == 1)
            Settings[S_VOLTAGEMIN] --;
          //if(ROW==2) Settings[S_TEMPERATUREMAX]--;
          if (ROW == 3) {
            Settings[S_BLINKINGHZ] --;
            if (Settings[S_BLINKINGHZ] < 1)
              Settings[S_BLINKINGHZ] = 1;
          }
        }
        ...
        if (configPage == 4 && COL == 3) {
          ...
          if (ROW == 4) {
            Settings[S_RSSIMAX] = rssiADC; // set MAX RSSI signal received (tx ON and rx near to tx)
          }
        }
        ...
        if (configPage == 6 && COL == 3) {
          if (ROW == 1)
            Settings[S_DISPLAYGPS] = !Settings[S_DISPLAYGPS];
          if (ROW == 2)
            Settings[S_COORDINATES] = !Settings[S_COORDINATES];
          if (ROW == 3)
            Settings[L_CALLSIGNPOSITIONDSPL] = !Settings[L_CALLSIGNPOSITIONDSPL];
        }

        if (configPage == 7 && COL == 3) {
          if (ROW == 1)
            Settings[S_RESETSTATISTICS] = !Settings[S_RESETSTATISTICS];
          if (ROW == 2)
            Settings[S_HEADING360] = !Settings[S_HEADING360];
          if (ROW == 3)
            Settings[S_UNITSYSTEM] = !Settings[S_UNITSYSTEM];
          if (ROW == 4) {
        ...
        if (configPage == 8) {
          if (ROW == 4) {
            ...
            if (COL == 2)
              Settings[screen_pos_item_pointer + 2] = !Settings[screen_pos_item_pointer + 2]; // Display/Hide
            if (COL == 3) {
              if ((Settings[screen_pos_item_pointer] > 1) && (Settings[screen_pos_item_pointer] != 255))
                Settings[screen_pos_item_pointer] --; // subtract 1 line
            }
            if (COL == 4) {
              if ((Settings[screen_pos_item_pointer + 1] > 1) && (Settings[screen_pos_item_pointer + 1] != 255))
                Settings[screen_pos_item_pointer + 1] --; // subtract 1 column
            }
          }
          if ((ROW == 7) && (COL == 1)) {
        ...
        if (configPage == 3 && COL == 3) {
          ...
          if (ROW == 3) {
            Settings[S_BLINKINGHZ] ++;
            if (Settings[S_BLINKINGHZ] > 10)
              Settings[S_BLINKINGHZ] = 10;
          }
        }
        ...
        if (configPage == 5 && COL == 3) {
          if (ROW == 1)
            accCalibrationTimer = CALIBRATION_DELAY;
          if (ROW == 5)
            magCalibrationTimer = CALIBRATION_DELAY;
          if (ROW == 7)
            eepromWriteTimer = EEPROM_WRITE_DELAY;
        }
          ...
          if (ROW == 4) {
            ...
            if (COL == 3) {
              if (Settings[S_VIDEOSIGNALTYPE]) {
                if (Settings[screen_pos_item_pointer] < 15)
                  Settings[screen_pos_item_pointer] ++; // add 1 line (Max 15 for PAL)
              }
              else if (Settings[screen_pos_item_pointer] < 13)
                Settings[screen_pos_item_pointer] ++; // add 1 line (Max 13 for NTSC)
            }
            if (COL == 4) {
              if (Settings[screen_pos_item_pointer + 1] < 25)
                Settings[screen_pos_item_pointer + 1] ++; // add 1 column
            }
          }
          if ((ROW == 7) && (COL == 1)) {
            WriteScreenLayoutDefault(); // Back and save to all default positions
