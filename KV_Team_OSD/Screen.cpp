  ...
  if (!(MW_STATUS.sensorActive & mode_osd_switch)) {
    if (Settings[L_MOTORARMEDPOSITIONDSPL]) {
      if (!armed) {
        ...
        screenBuffer[4] = 0;
        MAX7456_WriteString(screenBuffer,
          ((Settings[L_MOTORARMEDPOSITIONROW] - 1) * 30) + Settings[L_MOTORARMEDPOSITIONCOL]);
      }
      else if (armed) {
  ...
  if (Settings[L_VOLTAGEPOSITIONDSPL]) {
    ...
    if (!(MW_STATUS.sensorActive & mode_osd_switch) || (voltage <= (Settings[S_VOLTAGEMIN] + 2))) {
      ...
      if (Settings[L_MAINBATLEVEVOLUTIONDSPL]) {
        ...
        if (voltage < BATTEV1)
          screenBuffer[0] = SYM_BATT_EMPTY;
        else if (voltage < BATTEV2)
          screenBuffer[0] = SYM_BATT_1;
        else if (voltage < BATTEV3)
          screenBuffer[0] = SYM_BATT_2;
        else if (voltage < BATTEV4)
          screenBuffer[0] = SYM_BATT_3;
        else if (voltage < BATTEV5)
          screenBuffer[0] = SYM_BATT_4;
        else if (voltage < BATTEV6)
          screenBuffer[0] = SYM_BATT_5;
        else
          screenBuffer[0] = SYM_BATT_FULL; // Max charge icon
      }
      else {
      ...
      screenBuffer[1] = 0;
      MAX7456_WriteString(screenBuffer,
        ((Settings[L_VOLTAGEPOSITIONROW] - 1) * 30) + Settings[L_VOLTAGEPOSITIONCOL] - 1);
    }
  }
}
  ...
  if (!(MW_STATUS.sensorActive & mode_osd_switch)) {
    if (Settings[L_CURRENTTHROTTLEPOSITIONDSPL]) {
      if (MwRcData[THROTTLESTICK] > HighT)
        HighT = MwRcData[THROTTLESTICK] - 5;
      ...
      screenBuffer[2] = 0;
      if (CurThrottle > 95)
        screenBuffer[0] = SYM_THR_POINTER_TOP;
      else if (CurThrottle > 90)
        screenBuffer[0] = SYM_THR_POINTER_4;
      else if (CurThrottle > 85)
        screenBuffer[0] = SYM_THR_POINTER_3;
      else if (CurThrottle > 80)
        screenBuffer[0] = SYM_THR_POINTER_2;
      else if (CurThrottle > 75)
        screenBuffer[0] = SYM_THR_POINTER_1;
      else if (CurThrottle > 70)
        screenBuffer[0] = SYM_THR_POINTER_BOT;
      else if (CurThrottle > 65)
        screenBuffer[0] = SYM_THR_POINTER_DIV_LOW; // High throttle % Row 1
      else
        screenBuffer[0] = ' ';
      screenBuffer[1] = SYM_THR_SCALE_MAX;
      MAX7456_WriteString(screenBuffer,
        ((Settings[L_CURRENTTHROTTLEPOSITIONROW] - 1) * 30) + Settings[L_CURRENTTHROTTLEPOSITIONCOL]);
      if (CurThrottle > 70)
        screenBuffer[0] = ' ';
      else if (CurThrottle > 65)
        screenBuffer[0] = SYM_THR_POINTER_DIV_HIGH;
      else if (CurThrottle > 60)
        screenBuffer[0] = SYM_THR_POINTER_TOP;
      else if (CurThrottle > 55)
        screenBuffer[0] = SYM_THR_POINTER_4;
      else if (CurThrottle > 50)
        screenBuffer[0] = SYM_THR_POINTER_3;
      else if (CurThrottle > 45)
        screenBuffer[0] = SYM_THR_POINTER_2;
      else if (CurThrottle > 40)
        screenBuffer[0] = SYM_THR_POINTER_1;
      else if (CurThrottle > 35)
        screenBuffer[0] = SYM_THR_POINTER_BOT;
      else if (CurThrottle > 30)
        screenBuffer[0] = SYM_THR_POINTER_DIV_LOW; // Mid throttle % Row 2
      screenBuffer[1] = SYM_THR_SCALE_MED;
      MAX7456_WriteString(screenBuffer,
        ((Settings[L_CURRENTTHROTTLEPOSITIONROW] - 1) * 30) + Settings[L_CURRENTTHROTTLEPOSITIONCOL] + LINE);

      if (CurThrottle > 35)
        screenBuffer[0] = ' ';
      else if (CurThrottle > 30)
        screenBuffer[0] = SYM_THR_POINTER_DIV_HIGH;
      else if (CurThrottle > 25)
        screenBuffer[0] = SYM_THR_POINTER_TOP;
      else if (CurThrottle > 20)
        screenBuffer[0] = SYM_THR_POINTER_4;
      else if (CurThrottle > 15)
        screenBuffer[0] = SYM_THR_POINTER_3;
      else if (CurThrottle > 10)
        screenBuffer[0] = SYM_THR_POINTER_2;
      else if (CurThrottle > 5)
        screenBuffer[0] = SYM_THR_POINTER_1; // Minimum throttle % Row 3
      else
        screenBuffer[0] = SYM_THR_POINTER_BOT;
      screenBuffer[1] = SYM_THR_SCALE_LOW;
      MAX7456_WriteString(screenBuffer,
        ((Settings[L_CURRENTTHROTTLEPOSITIONROW] - 1) * 30) + Settings[L_CURRENTTHROTTLEPOSITIONCOL] + LINE + LINE);
...
void displaypMeterSum(void)
{
  ...
  if (Settings[L_PMETERSUMPOSITIONDSPL]) {
    ...
    if (!(MW_STATUS.sensorActive & mode_osd_switch))
      MAX7456_WriteString(screenBuffer,
        ((Settings[L_PMETERSUMPOSITIONROW] - 1) * 30) + Settings[L_PMETERSUMPOSITIONCOL]);
  }
}
...
void displayHeading(void)
{
  ...
  if (Settings[L_MW_HEADINGPOSITIONDSPL]) {
    ...
    if (!(MW_STATUS.sensorActive & mode_osd_switch))
      MAX7456_WriteString(screenBuffer,
        ((Settings[L_MW_HEADINGPOSITIONROW] - 1) * 30) + Settings[L_MW_HEADINGPOSITIONCOL]);
  }
}
...
void displayHeadingGraph(void)
{
  if (!(MW_STATUS.sensorActive & mode_osd_switch) || (!GPS.fix)) {
    if (Settings[L_MW_HEADINGGRAPHPOSDSPL]) {
      ...
      screenBuffer[1] = 0;
      MAX7456_WriteString(screenBuffer,
        ((Settings[L_MW_HEADINGGRAPHPOSROW] - 1) * 30) + Settings[L_MW_HEADINGGRAPHPOSCOL] - LINE + 2);
    }
  }
}
...
void displayIntro(char position)
{
  ...
  if (Settings[L_CALLSIGNPOSITIONDSPL]) {
    ...
    if (!BlinkAlarm)
      MAX7456_WriteString(screenBuffer, position + 130 + LINE + LINE + LINE + LINE + LINE + LINE + LINE + LINE);
    ; // Call Sign on the beginning of the transmission (blink at sel freq)
  }
}
  ...
  if (!(MW_STATUS.sensorActive & mode_osd_switch) || rssi <= (Settings[S_RSSI_ALARM] - 10)) {
    if (Settings[S_COORDINATES]) {
      if (Settings[L_MW_GPS_LATPOSITIONDSPL]) {
        ...
        MAX7456_WriteString(screenBuffer,
          ((Settings[L_MW_GPS_LATPOSITIONROW] - 1) * 30) + Settings[L_MW_GPS_LATPOSITIONCOL]);
      }
      if (Settings[L_MW_GPS_LONPOSITIONDSPL]) {
        screenBuffer[0] = SYM_LON;
        FormatGPSCoord(GPS.longitude, screenBuffer + 1, 4, 'E', 'W');
        MAX7456_WriteString(screenBuffer,
          ((Settings[L_MW_GPS_LONPOSITIONROW] - 1) * 30) + Settings[L_MW_GPS_LONPOSITIONCOL]);
      }
    }
  }
}
...
void displayNumberOfSat(void)
{
  if (Settings[L_GPS_NUMSATPOSITIONDSPL]) {
    ...
    if (!(MW_STATUS.sensorActive & mode_osd_switch))
      MAX7456_WriteString(screenBuffer,
        ((Settings[L_GPS_NUMSATPOSITIONROW] - 1) * 30) + Settings[L_GPS_NUMSATPOSITIONCOL]);
  }
}
...
void displayAltitude(void)
{
  ...
  if (Settings[L_MW_ALTITUDEPOSITIONDSPL]) {
    ...
    if (!(MW_STATUS.sensorActive & mode_osd_switch) || altitude >= (Settings[S_VOLUME_ALT_MAX] * 10)
      || flyTime > 30 && altitude < (Settings[S_VOLUME_ALT_MIN])) {
      if (altitude >= (Settings[S_VOLUME_ALT_MAX] * 10) && !BlinkAlarm
        || flyTime > 60 && altitude < (Settings[S_VOLUME_ALT_MIN]) && !BlinkAlarm)
        return;
      screenBuffer[0] = SYM_ALT;
      itoa(altitude, screenBuffer + 1, 10);
      MAX7456_WriteString(screenBuffer,
        ((Settings[L_MW_ALTITUDEPOSITIONROW] - 1) * 30) + Settings[L_MW_ALTITUDEPOSITIONCOL]);
    }
  }
}
...
void displayClimbRate(void)
{
  if (!(MW_STATUS.sensorActive & mode_osd_switch)) {
    if (Settings[L_CLIMBRATEPOSITIONDSPL]) {
      ...
      if (MW_ALT.Vario > 100)
        screenBuffer[0] = SYM_POS_CLIMB3;
      else if (MW_ALT.Vario > 70)
        screenBuffer[0] = SYM_POS_CLIMB2;
      else if (MW_ALT.Vario > 50)
        screenBuffer[0] = SYM_POS_CLIMB1;
      else if (MW_ALT.Vario > 30)
        screenBuffer[0] = SYM_POS_CLIMB;
      else if (MW_ALT.Vario < -100)
        screenBuffer[0] = SYM_NEG_CLIMB3;
      else if (MW_ALT.Vario < -70)
        screenBuffer[0] = SYM_NEG_CLIMB2;
      else if (MW_ALT.Vario < -50)
        screenBuffer[0] = SYM_NEG_CLIMB1;
      else if (MW_ALT.Vario < -30)
        screenBuffer[0] = SYM_NEG_CLIMB;
      else
        screenBuffer[0] = SYM_ZERO_CLIMB;
      screenBuffer[1] = 0;

      MAX7456_WriteString(screenBuffer,
        ((Settings[L_CLIMBRATEPOSITIONROW] - 1) * 30) + Settings[L_CLIMBRATEPOSITIONCOL] + 3);
      ...
      MAX7456_WriteString(screenBuffer,
        ((Settings[L_CLIMBRATEPOSITIONROW] - 1) * 30) + Settings[L_CLIMBRATEPOSITIONCOL]);
    }
  }
}
...
void displayDistanceToHome(void)
{
  if (Settings[L_GPS_DISTANCETOHOMEPOSDSPL]) {
    ...
    if (!(MW_STATUS.sensorActive & mode_osd_switch) || GPS.distanceToHome >= (Settings[S_VOLUME_DIST_MAX] * 100))
      MAX7456_WriteString(screenBuffer,
        ((Settings[L_GPS_DISTANCETOHOMEPOSROW] - 1) * 30) + Settings[L_GPS_DISTANCETOHOMEPOSCOL]);
  }
}
...
{
  if (!(MW_STATUS.sensorActive & mode_osd_switch)
    || (rssi <= (Settings[S_RSSI_ALARM])
      || (voltage <= (Settings[S_VOLTAGEMIN] + 1) || GPS.distanceToHome >= (Settings[S_VOLUME_DIST_MAX] * 100)))) {
    if (Settings[L_GPS_DIRECTIONTOHOMEPOSDSPL]) {
      ...
      screenBuffer[2] = 0;
      MAX7456_WriteString(screenBuffer,
        ((Settings[L_GPS_DIRECTIONTOHOMEPOSROW] - 1) * 30) + Settings[L_GPS_DIRECTIONTOHOMEPOSCOL]);
    }
  }
}
...
void displayAngleToHome(void)
{
  if (Settings[L_GPS_ANGLETOHOMEPOSDSPL]) {
    ...
    if (!(MW_STATUS.sensorActive & mode_osd_switch))
      MAX7456_WriteString(screenBuffer,
        ((Settings[L_GPS_ANGLETOHOMEPOSROW] - 1) * 30) + Settings[L_GPS_ANGLETOHOMEPOSCOL]);
  }
}
  ...
  if (ROW < 10) {
    if (configPage == 1) {
      ...
      if (COL == 4)
        COL = 3;
      if (COL == 1)
        cursorpos = (ROW + 2) * 30 + 10;
      if (COL == 2)
        cursorpos = (ROW + 2) * 30 + 10 + 6;
      if (COL == 3)
    ...
    if (configPage == 7) {
      ...
      if (ROW == 5)
        ROW = 10;
      if (ROW == 3)
        cursorpos = (ROW + 2) * 30 + 10 + 6 + 6 - 2; // Metric/Imperial string longer
      else
        cursorpos = (ROW + 2) * 30 + 10 + 6 + 6;
    }
    ...
    if (configPage == 8) {
      ...
      if (ROW == 8)
        ROW = 10;
      if (COL == 1)
        cursorpos = (ROW + 2) * 30 + 2; // Item
      if (COL == 2)
        cursorpos = (ROW + 2) * 30 + 15; // On/Off
      if (COL == 3)
        cursorpos = (ROW + 2) * 30 + 20; // LINE
      if (COL == 4)
        cursorpos = (ROW + 2) * 30 + 24; // COL
    }
...
void displayConfigScreen(void)
{
  ...
  if (configPage == 4) {
    ...
    if (rssiTimer > 0) {
      MAX7456_WriteString(itoa(rssiTimer, screenBuffer, 10), YAWD - 5);
    }
  ...
  if (configPage == 8) {
    ...
    if (screenrow != 255)
      MAX7456_WriteString(itoa((screenrow), screenBuffer, 10), LINE07 + 21); // Line position
    else
      MAX7456_WriteString_P(configMsgNoAct, LINE07 + 21);
    if (screencol != 255)
      MAX7456_WriteString(itoa(screencol, screenBuffer, 10), LINE07 + 25); // Col position
    else
      MAX7456_WriteString_P(configMsgNoAct, LINE07 + 25);
...
void displaySensor(void)
{
  if (!(MW_STATUS.sensorActive & mode_osd_switch)) {
    if (Settings[L_SENSORPOSITIONDSPL]) {
      ...
      // Sensor Active
      if (MW_STATUS.sensorActive & mode_baro)
        screenBuffer[1] = SYM_BAR_ON;
      if (MW_STATUS.sensorActive & mode_mag)
        screenBuffer[2] = SYM_MAG_ON;
      if ((MW_STATUS.sensorActive & mode_stable)
        | (MW_STATUS.sensorActive & mode_horizon && (MwRcData[PITCHSTICK] > 1450) && (MwRcData[PITCHSTICK] < 1550))) {
        screenBuffer[3] = SYM_ACC_ON;
      }
      MAX7456_WriteString(screenBuffer, ((Settings[L_SENSORPOSITIONROW] - 1) * 30) + Settings[L_SENSORPOSITIONCOL]);
    }
  }
...
void displayGPSMode(void) // Moves together with Settings[L_SENSORPOSITION].
{
  if ((GPS.fix) & (!(MW_STATUS.sensorActive & mode_osd_switch) & (Settings[L_SENSORPOSITIONDSPL])))
    ...
    else if (MW_STATUS.sensorActive & mode_gpshold)
...
void displayautoPilot(void)
{
  if (GPS.fix & flyTime > 7)
    if (MW_STATUS.sensorActive & mode_gpshome || MW_STATUS.sensorActive & mode_gpshold) { // Autopilot warning override SMART SW
      ...
      if (BlinkAlarm)
        MAX7456_WriteString(screenBuffer,
          ((Settings[L_SENSORPOSITIONROW] - 1) * 30) + Settings[L_SENSORPOSITIONCOL] + LINE);
    }
}
